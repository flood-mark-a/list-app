...
class ListKafkaResourceIT {
    ...
    void setup() {
        ...
        ListKafkaResource kafkaResource = new ListKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/list-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/list-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
